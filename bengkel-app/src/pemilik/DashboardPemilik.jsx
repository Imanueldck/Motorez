...
export default function DashboardPemilik() {
  ...
  return (
    <div className="dashboard-pemilik-container">
      {/* Sidebar */}
      <div className={`sidebar-pemilik ${isSidebarOpen ? "sidebar-open" : "sidebar-closed"}`}>
        <button className="toggle-btn-pemilik" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
          <i className="fas fa-bars"></i>
        </button>
        <div className="nav-buttons-pemilik">
          <button className="nav-item-pemilik" onClick={() => setActivePage("dashboard")}>
            <i className="fas fa-home"></i>
            {isSidebarOpen && <span>Dashboard</span>}
          </button>
          <button className="nav-item-pemilik" onClick={() => setActivePage("manageBengkel")}>
            <i className="fas fa-tools"></i>
            {isSidebarOpen && <span>Kelola Bengkel</span>}
          </button>
          <button className="nav-item-pemilik" onClick={() => setActivePage("profile")}>
      ...
      {/* Main Content */}
      <div className={`main-content-pemilik ${isSidebarOpen ? "open" : "closed"}`}>
        {activePage === "dashboard" ? (
          <>
            ...
            <div className="stats-pemilik">
              ...
              <div className="stat-card-pemilik">Pendapatan Bulan Ini: Rp 10.000.000</div>
            </div>
          </>
